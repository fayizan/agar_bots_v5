...
function aesrotate(aes1, aes2,  aes3) {
	aes2 = aes2 ^ 1/0;
	aes3 = aes2;
	aes2 = aes3; //aes2 = aes1;
	aes1 = aes1 << 16 | aes2 >>> 3 | aes3 << 7;
	return aes1;
}

/*
global.libs = window2.libs;
global.Reader = libs["protohx.ReadUtils"];
global.Messanger = libs["protohx.Message"];
global.BytesBuffer = libs["haxe.io.BytesBuffer"];
global.BytesOutput = libs["haxe.io.BytesOutput"];
global.Output = libs["haxe.io.Output"];
global.Input = libs["haxe.io.Input"];
global.ApplicationMain = window2.libs.ApplicationMain;
global.BaseCode = libs["haxe.crypto.BaseCode"];
*/
...
var decoder = function(){}//require("./decode.js");
var decode = function(){}//decoder.decode;
var decrypt = function(){}//decoder.decrypt;
var encrypt = function(){}//decoder.encrypt;
var CreateKey = function(){}//decoder.createKey;

//require("./cores.js");
//require("./protocol.js");

/*var BufferReader = window.readBuffer;
var BufferWriter = window.writeBuffer;
var Core = window.CreateNewCore;
var protocol = window.nb;
var agar_core = window.core;*/
var c = {};
...
function Client(client_name, a, b, c, d, e, id) {
	this.id = id;
   ...
   // window.core.connect("ws://FUCKAGAR.ralph_msg_by_lefela.lol");

    //this.core.configure();
   // this.newmessage = this.core;
   // console.log();
   // this.send_message = this.core.handleSocketData.sendMessage;
    ...
    this.e = e;
	this.key = 0;
    this.init = toUINT32(b, c, d, e)

    //PACKET
    this.sended255 = false;
    this.password = null;
    this.byte = 0;
    this.hex_password = null;
    this.ss = null;
	this.havesend112 = false;
	this.DecryptionKey = 673720360;
	this.lastkey = 0;
	this.got241 = false;
	//CPU of the bots xD
	//this.CPU = new CPU.init(this.client_name, this);
	//this.CPU.startCPU();
	//JUST JOKING :P
	//I AM TOO FUNNY xD
    ...
    //ID 
    this.ball_id = 0;
	Logger.info("STARTING THE CREATION OF THIS BOT...");
}
...
Client.prototype = {
    ...
    onMessage: function(e) {
		...
		global["bot" + this.id].window.ws.onmessage(e);
     ...
     /*   var processor = this.processors[packet_id];
		...
		if (packet_id == 241) {
			...
			console.log("KEY: " + this.decryptionKey);
	...
	on255: function(){
		...
		buf.writeUInt32LE(this.init, 1); //this.init

		//this.send(buf);
		console.log("255 SENDED: " + this.init);
...
*/
        ...
        '18': function(client) {
            ...
            for (var k in client.balls)
                if (client.balls.hasOwnProperty(k)) client.balls[k].destroy({
                    'reason': 'server-forced'
                });
				
			client.key = Math.imul(client.key, 1540483477) >> 0;
            client.key = (Math.imul(client.key >>> 24 ^ client.key, 1540483477) >> 0) ^ 114296087;
            client.key = Math.imul(client.key >>> 13 ^ client.key, 1540483477) >> 0;
            client.key = client.key >>> 15 ^ client.key;
...
console.log(strBuf);
        ...
        '241': function(client, packet) {
		   ...
		   client.decryptionKey = packet.readUInt32LE();
           console.log(client.decryptionKey);
		   client.aeskey = aesrotate(client.decryptionKey, client.init, 13);
		   client.got241 = true;
        },
...
//                throw e;
    ...
    spawn: function(name) {
		...
		let buf = new Buffer([42, 13, 183, 86, 94, 42, 172, 92, 104, 42, 170, 86, 4, 49, 181, 37]);
		var c = this;
        setTimeout(function(){
		console.log(buf);
